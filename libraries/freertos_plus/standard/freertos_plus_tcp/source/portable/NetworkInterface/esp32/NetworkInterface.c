    ...
    if (xInterfaceState == INTERFACE_DOWN) {
        ...
        if( ret == ESP_OK )
        {
            if( mode == WIFI_MODE_STA )
            {
                ...
                if (ret != ESP_OK) {
                    ESP_LOGE(TAG, "STA mode: Failed to tx buffer %p, len %d, err %d", pxNetworkBuffer->pucEthernetBuffer, pxNetworkBuffer->xDataLength, ret);
                }
            }
            else if( mode == WIFI_MODE_AP )
            {
                ret = esp_wifi_internal_tx(ESP_IF_WIFI_AP, pxNetworkBuffer->pucEthernetBuffer, pxNetworkBuffer->xDataLength);
                if (ret != ESP_OK) {
                    ESP_LOGE(TAG, "AP mode: Failed to tx buffer %p, len %d, err %d", pxNetworkBuffer->pucEthernetBuffer, pxNetworkBuffer->xDataLength, ret);
                }
            }
            else
            {
                ESP_LOGE(TAG, "Unexpected mode");
                ret = ESP_FAIL;
            }
        }
        else
        {
            ESP_LOGE(TAG, "Unexpected get mode error");
            ret = ESP_FAIL;
        }
    }
